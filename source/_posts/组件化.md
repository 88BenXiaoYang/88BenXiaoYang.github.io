...
小结：在`iOS`领域里，一定是组件化的中间件为`openURL`提供服务，而不是`openURL`方式为组件化提供服务。如果在给`App`实施组件化方案的过程中是基于`openURL`方案的话，有一个致命的缺陷，非常规对象(不能被字符串化到`URL`中的对象，如`UIImage`)无法参与本地组件间的调度。
